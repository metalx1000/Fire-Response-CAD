...
$(document).ready(function(){
  checkUnit();
  update();
  //update if monitoring call
  if(!window.location.href.includes("byid.php")){
...
function refresh(data){
  if(data.length < 1){
    ...
    $("#notes").html("Call Does Not Exist");
  }
  ...
  for(var i=0;i<data.length;i++){
    ...
    if(localStorage.calllid != callid || loc == ""){
      ...
      if(id !== null){
        var status = "Viewing Past Call";
      }else{
        var status = "Monitoring " + unit;
      }

      //get address
      $("#unitInfo").html("<a href='changeunit.php'>"+status+"</a>");
      loc = data[i].loc.toUpperCase();
      loc = loc.replace("(AP)","");//For memory care 

      //check for apartment number
      if(data[i].apt != ""){
        var apt = data[i].apt.toUpperCase();
        var loc2 = loc + " APT#" + apt;
      }
      ...
      msg+=data[i].cmt.split("RUN CARD")[0];
